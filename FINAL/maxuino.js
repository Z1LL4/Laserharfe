...
function stepperStep(e, a, b) {
    var d = 0;
    if(b>0)var d = 1;
    b=Math.abs(b);
    var c = Math.floor(b/255);
    b=b%255; 
    outlet(0, 240, 103,1, a, b, c, d, 247);
}

function stepperSpeed(e, a, b) {
var c = Math.floor(b/255);
    b=b%255;
            ...
            if (i<3) {
             ...
             if (i == 3) {
                ...
                if (command[0] > 143 && command[0] < 160) {   //digital pin inputs
                    ...
                    outlet(0, "/board", "/version", command[1]+command[2]/10);
                }
            }        
        }
    }
...
function pinStatus() {
    outlet(0, "/"+command[2], "/state", command[3], command[4]);
}
...
function boardStatusD() {
    ...
    for (var i=2; i<command.length; i++) {
        ...
        if(command[i]==127) {
            outlet(0, "/board/digital","/"+ pinNum, "/info",tempArray);
